...
def _collect_php(url: str, s: requests.Session) -> tuple[str, dict]:
    print(f"🌐  GET {url}")
    ...
    for tag in soup.find_all("a"):
        ...
        if href and href.endswith(".php"):
            ...
            try:
                print(f"📥  Скачиваю PHP {php_url}")
                php_code = s.get(php_url, timeout=7).text
                code.append(php_code)
            except Exception as e:
                print(f"⚠️  Не удалось скачать {php_url}: {e}")

    print(f"🗂️  Собрано {len(code)} PHP фрагментов")
    return "\n".join(code), r.headers
...
def run(domain: str, tg_token: str, tg_chat: int) -> None:
    ...
    for line in proxies_list:
        ...
        with requests.Session() as s:
            ...
            try:
                php, headers = _collect_php(domain, s)
            except Exception as e:
            ...
            if found:
                ...
                if tg_token and tg_chat:
                    try:
                        ...
                        modules.telegram.send_message(tg_token, tg_chat, f"Найдены php фреймы для сайта: {domain}\n" + "".join(map(str, reses)))
                    except Exception as e:
